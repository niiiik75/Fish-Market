 ...
 * @version
 */
(function($,window,document,undefined){var defaultOpts={autoInit:true,updateOnResize:true},heightRegEx=/\d+h/i,widthRegEx=/\d+w/i,dpiRegEx=/([\d\.]+)x\b/i;$.srcset=$.extend(defaultOpts,$.srcset||{});function getSrcSetOptions(srcsetData){return{url:srcsetData.split(" ").length>0?srcsetData.split(" ")[0]:undefined,w:widthRegEx.test(srcsetData)?parseInt(widthRegEx.exec(srcsetData)[0]):Infinity,h:heightRegEx.test(srcsetData)?parseInt(heightRegEx.exec(srcsetData)[0]):Infinity,dpi:parseFloat(dpiRegEx.test(srcsetData)?dpiRegEx.exec(srcsetData)[1]:1)};}function setSrc(img){var srcsetdata=img.data('srcset'),srcsets=srcsetdata.split(","),data=[];for(var i=0;i<srcsets.length;i++){var parsed=getSrcSetOptions(srcsets[i].replace(/^\s*|\s*$/gi,''));if(!parsed.url){console.log("Couldn't parse URL; got %o",parsed);continue;}data.push(parsed);}if(data.length===0){console.log("Couldn't parse srcset data for %o",img);return false;}img.data('srcset-sizes',data);return data;}function updateSrc(img){if(!img.data('srcset-sizes')){return;}var h=$(window).height(),w=$(window).width(),dpi=window.devicePixelRatio||1;var data=img.data('srcset-sizes');filteredData=$.grep(data,function(el,idx){return(el.w>=w&&el.h>=h&&el.dpi>=dpi);});var best={data:undefined,diff:Infinity};if(filteredData.length===1){best.data=filteredData[0];}if(filteredData.length===0){data.map(function(e,i){var wDiff=w-e.w===-Infinity?0:w*dpi-e.w,hDiff=h-e.h===-Infinity?0:h*dpi-e.h;e.diff=wDiff+hDiff;return e;}).sort(function(a,b){return a.diff<b.diff;}).forEach(function(val,idx){if(best.data&&best.data.diff<=0){return;}best.data=val;});}else{for(var i=0;i<filteredData.length;i++){var wDiff=filteredData[i].w-w===Infinity?0:filteredData[i].w*dpi-w,hDiff=filteredData[i].h-h===Infinity?0:filteredData[i].h*dpi-h;var diff=wDiff+hDiff;if(diff<best.diff){best.diff=diff;best.data=filteredData[i];}}}data=best.data;if(!data||!data.url){console.log("No src found for %o",img);return;}var imageUrl=(img.data('srcset-base')||"")+data.url+(img.data('srcset-ext')||"");if(img.attr('src')===imageUrl){return;}img.trigger("beforeSrcReplace");img.attr('src',imageUrl);img.trigger("srcReplaced");}$.fn.srcset=function(opts){options=$.extend({},$.srcset,opts||{});$(this).each(function(){setSrc($(this));updateSrc($(this));});if(options.updateOnResize){$(window).on('resize',function(){$(this).each(function(){updateSrc($(this));});}.bind(this));$(window);}};$(function(){if($.srcset.autoInit){$("img[data-srcset]").srcset();}});})(window.jQuery,window,document);